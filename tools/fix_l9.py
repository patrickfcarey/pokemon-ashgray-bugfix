...
for t in (0x726A98, 0x726AA8):
    var_at = t + 6
    if bytes(ag[var_at:var_at+2]) == bytes([0x96, 0x61]):
        print(f'trigger @0x{t:06X}: already re-gated on 0x6196 — skip')
        continue
    ...
    changed = True
if not changed:
    print('L9 already applied on both triggers — no change')
    raise SystemExit(0)
