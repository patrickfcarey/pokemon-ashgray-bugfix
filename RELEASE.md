...
The fork ships as an **IPS patch** (with an equivalent **BPS patch** alongside it) you apply to a clean FireRed ROM. It does **not** include any copyrighted ROM. Both formats produce the identical fork — use whichever your patcher prefers.
...
- **The early-forest scripted battles do *not* freeze (earlier report retracted).** An earlier note flagged the Caterpie and Pidgeotto scripted battles as freezing during the battle-end fade. Both were re-tested in-engine — the Caterpie battle directly and at length (~19 runs + a 10-timing RNG sweep), and the Pidgeotto battle staged live and swept (another 10 timings) — and **neither freezes**: every run either kept playing normally or completed its battle-end fade cleanly (whiteout→home, or return to the field). The original "freeze" was a testing artifact (a battle menu waiting for input that looked stuck). Details in `audit/f1-battle-freeze-retest.md`. The fork has **no remaining known game-blocking bug** — we can't prove a universal negative over every RNG path, so in the unlikely event you ever hit a hard hang there, a reset and retry will clear it.
