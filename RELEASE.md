...
- **The early-forest scripted battle can still freeze.** The Pidgeotto/Caterpie scripted battles near the start hang non-deterministically during the battle-end fade. This lives inside Ash Gray's modified battle engine (hand-written ASM), which is beyond safe patching — we reproduced it but did **not** fix it. If it freezes, reset and retry; the same save completes on another attempt.
